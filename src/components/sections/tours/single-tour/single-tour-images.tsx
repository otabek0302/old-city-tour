  ...
  return (
    <div className="mb-6">
      <div className="relative w-full h-[520px] mb-2 border border-border rounded-xl overflow-hidden">
        <Image src={getImageURL(mainImage.image.url)} fill priority alt={typeof mainImage.image === "object" && mainImage.image.alt ? mainImage.image.alt : "Tour image"} className="object-cover object-center" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" onError={() => setMainImage(images[0])} />
      </div>
      <div className="flex gap-2 overflow-x-auto">
        {images.map((img: ImageItem, idx: number) => (
          <button key={img.id || idx} onClick={() => setMainIdx(idx)} className={`relative h-32 w-52 border-2 rounded-xl overflow-hidden flex items-center justify-center transition-all ${mainIdx === idx ? "border-primary" : "border-border"}`} type="button">
            <Image src={getImageURL(img.image.url)} fill priority alt={typeof img.image === "object" && img.image.alt ? img.image.alt : `Thumbnail ${idx + 1}`} className="object-cover object-center" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
          </button>
        ))}
      </div>
    </div>
