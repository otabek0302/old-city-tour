...
async function getTours(cityId: number, locale: string) {
  try {
    const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000";
    console.log("Fetching tours for cityId:", cityId, "locale:", locale, "baseUrl:", baseUrl);
    const res = await fetch(`${baseUrl}/api/tours?where[cities][equals]=${cityId}&locale=${locale}`, { cache: "no-store" });
    console.log("Tours API response status:", res.status);
    if (!res.ok) {
      console.log("Tours API error:", res.statusText);
      return [];
    }
    const data = await res.json();
