        ...
        {hotels.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {hotels.map((hotel) => {
              ...
              return (
                <Link key={hotel.id} href={`/hotels/${hotelSlug}`} className="block group">
                  <Card className="bg-background border border-border rounded-2xl overflow-hidden hover:shadow-lg transition-all duration-200 group-hover:border-primary cursor-pointer">
                    {hotel.images?.[0]?.image && (
                      <div className="relative h-48 overflow-hidden">
                        <Image src={hotel.images[0].image.url} alt={hotel.name} fill className="object-cover transition-transform duration-200 group-hover:scale-105" />
                      </div>
                    )}
                    <CardContent className="p-6">
                      <h2 className="text-copy text-lg font-semibold mb-3 group-hover:text-primary transition-colors">{hotel.name}</h2>
